...
models.py — здесь ты определяешь, что такое "продажа" (Sale), кто такой "менеджер", "страна", "продукт".

Все поля ссылаются друг на друга через foreign key — удобно для аналитики и не нарушает целостность данных
**models.py:**
...
В файле resources/sale.py реализованы функции для создания, чтения, обновления и удаления продаж (SaleListResource, SaleResource).

Функция sale_to_dict возвращает не просто id менеджера, а целый объект {id, name} — чтобы фронтенд мог красиво всё показать.
...
Важно: В frontend/package.json обязательно должна быть эта строчка
...
# Как это всё связано?
Frontend делает fetch-запросы к API backend (например, /api/v1/sales или /api/v1/stats/sales?group=country)

Backend отдаёт нужные данные в формате JSON, уже подготовленные к выводу (например, название страны, имя менеджера).

Благодаря "proxy": "http://localhost:5000" все AJAX-запросы с фронта автоматически идут на backend, даже если работают на разных портах.

Интерфейс мгновенно обновляется, когда приходят новые данные — никакой перезагрузки страницы.
